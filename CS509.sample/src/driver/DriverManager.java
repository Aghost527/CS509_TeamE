...
//		Flights flights2 = resSys.getFlighs("TeamE",Arrival,time,false);//true means search by departure
...
//		Flights flights3 = resSys.getFlighs("TeamE",Arrival,time,false);//true means search by departure
			...
			if(!map.containsKey(f1.getArrival())){
				...
				for(Flight f2:map.get(f1.getArrival())){
					for(Flight f3:flights3){
						if(f2.getArrival().equals(f3.getDeparture())){
							Flights flight=new Flights();
							flight.add(f1);
							flight.add(f2);
							flight.add(f3);
